<div class="row mb2 respiro">
    <div class="col-md-4">
        <div class="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
            <div class="col p-4 d-flex flex-column position-static">
                
                <h4 class="mb-0">Gil Felipe da Silva</h4>
                <strong class="d-inline-block mb-2 text-primary">Analista Júnior @ Infinity</strong>
                <p class="card-text mb-auto">sabe dar print screen com o mouse</p>
                <a href="index.php?page=/templates/CVGil" class="stretched-link">Seguir para currículo</a>
            </div>
            <div class="col-auto d-none d-lg-block">
                <img src="../assets/thumbs/Gil.jpg" alt="Gil">
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
            <div class="col p-4 d-flex flex-column position-static">
                
                <h4 class="mb-0">Rodrigo Torres Savone</h4>
                <strong class="d-inline-block mb-2 text-primary">Analista Júnior @ 7comm</strong>
                <p class="card-text mb-auto">sabe dar print screen com o mouse</p>
                <a href="index.php?page=/templates/CVRodrigo" class="stretched-link">Seguir para currículo</a>
            </div>
            <div class="col-auto d-none d-lg-block">
                <img src="../assets/thumbs/Rodrigo.jpg" alt="Rodrigo">
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
            <div class="col p-4 d-flex flex-column position-static">
                
                <h4 class="mb-0">Leonardo de Sá Santos</h4>
                <strong class="d-inline-block mb-2 text-primary">Treinee @ 7comm</strong>
                <p class="card-text mb-auto">sabe dar print screen com o mouse</p>
                <a href="index.php?page=/templates/CVLeo" class="stretched-link">Seguir para currículo</a>
            </div>
            <div class="col-auto d-none d-lg-block">
                <img src="../assets/thumbs/Leo.jpg" alt="Leo">
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
            <div class="col p-4 d-flex flex-column position-static">
                
                <h4 class="mb-0">Felipe Seiji</h4>
                <strong class="d-inline-block mb-2 text-primary">Analista Júnior @ Infinity</strong>
                <p class="card-text mb-auto">sabe dar print screen com o mouse</p>
                <a href="index.php?page=/templates/CVSeiji" class="stretched-link">Seguir para currículo</a>
            </div>
            <div class="col-auto d-none d-lg-block">
                <img src="../assets/thumbs/Seiji.jpg" alt="Seiji">
            </div>
        </div>
    </div>
    <div class="col-md-4">
        <div class="row no-gutters border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
            <div class="col p-4 d-flex flex-column position-static">
                
                <h4 class="mb-0">Makaya Afonso</h4>
                <strong class="d-inline-block mb-2 text-primary">Desenvolvedor Júnior</strong>
                <p class="card-text mb-auto">sabe dar print screen com o mouse</p>
                <a href="index.php?page=/templates/CVMakaya" class="stretched-link">Seguir para currículo</a>
            </div>
            <div class="col-auto d-none d-lg-block">
                <img width='199px' height='199px' src="../assets/thumbs/makaya.jpeg" alt="makaya">
            </div>
        </div>
    </div>
</div>